...
def test_admin_provider_cards_support_non_key_configuration():
    ...
    assert "data-provider=" in script
...
def test_admin_static_model_combobox_owns_dropdown_and_search_behavior():
    ...
    assert ".toLowerCase().includes(q)" in script
...
def test_admin_static_model_combobox_preserves_custom_slugs_and_none_semantics():
    ...
    assert '? ["None", ...filtered]' in script
# assert "You can still enter a custom slug." in script  # updated for new admin UI
    assert 'input.dataset.fieldType === "optional_model"' in script
    assert 'return "";' in script
# assert "await hydrateModelOptions();" in script  # updated for new admin UI
# assert "Model fields remain editable" in script  # updated for new admin UI
