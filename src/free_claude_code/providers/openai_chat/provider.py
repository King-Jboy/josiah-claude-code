...
class OpenAIChatProvider(BaseProvider):
    ...
    def __init__(
        ...
        *,
        ...
        http_client = None
        # One shared HTTP/2 client across all keys (and for proxied providers):
        # api_key_provider swaps the credential per request on the same client,
        # so the connection pool is reused across rotations and concurrent
        # requests multiplex over one TCP+TLS connection per host. h2 comes from
        # httpx[http2]. Non-pool, non-proxy providers keep AsyncOpenAI's default
        # internal client (unchanged single-key behavior).
        if key_pool is not None or config.proxy:
        ...
        credential = (
            key_pool if key_pool is not None else (api_key_provider or self._api_key)
        )
    ...
    async def _open_upstream_stream(self, create_body: dict[str, Any]) -> Any:
        """Open the upstream SSE stream, routing through the key pool if present.

        Without a key pool this is a direct ``chat.completions.create`` call
        (unchanged single-key behavior). With a pool, the call is wrapped in the
        per-key 429 rotation lap; non-429 errors propagate to the existing
        admission / recovery handling untouched.
        """
        ...
        return await self._key_pool.create_with_rotation(
    ...
    async def _create_stream(
        ...
        while retry_session.can_attempt:
            ...
            try:
                ...
                stream = await self._open_upstream_stream(create_body)
