...
class Settings(BaseSettings):
    ...
    # Plural key pool (JSON array, e.g. ["k1","k2"]); takes precedence over
    # the singular key when non-empty. Enables per-key rate-limit rotation.
    ...
    nvidia_nim_rpm_per_key: int = Field(
        default=40, validation_alias="NVIDIA_NIM_RPM_PER_KEY"
    )
