...
Use Claude Code or OpenCode through your own provider-backed proxy.
...
- Launch Claude Code with `fcc-claude`, Codex with `fcc-opencode`, or Pi with ``.
...
For terminal use, start `fcc-server`, then run `fcc-claude`, `fcc-opencode`, or ``. Use the guides below for editor integrations.
